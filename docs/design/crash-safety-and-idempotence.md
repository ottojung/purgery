...
Processing uses staged sources and a run-local work area. The server invokes transform subprocesses. Transform programs are trusted to place outputs at configured paths; the server checks declared `expected_outputs` exist after the transform. `expected_outputs` are path patterns: relative patterns resolve against `<DESTINATION>`, absolute patterns are used as-is, and `{target_directory}` is allowed. Status files are published atomically before terminal phase publication. Recovery resumes processing runs from staged data and filesystem state.
...
`work_dir` owns incoming files, phase directories, manifests, leases, progress, status, and work areas. Transform outputs are written by the transform program itself. The destination for relative `expected_outputs` is the run destination recorded in `run.toml`; absolute `expected_outputs` are used as-is. The server does not move or commit transform outputs.
