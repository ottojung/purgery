...
GC owns expired server work state in every phase. Successful `done` requests keep `status.toml`, `run.toml`, and `manifest.toml` for `done_retention_secs`, but GC and finalization remove staged/control material such as `files/`, `work/`, `lease.toml`, `progress.toml`, `processor.lock`, and `*.tmp`. Failed requests may keep payload/work material for `failed_retention_secs` for inspection. Malformed, incompatible, or unknown request directories are retained only for `orphan_retention_secs`; GC logs that it cannot understand them and then removes them after the grace window.
